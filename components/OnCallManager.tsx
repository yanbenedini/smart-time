...
import { Siren, Plus, Trash2, Edit2, Calendar, Clock, AlertTriangle, X, UserCircle, Download, Upload, Search, Filter, FileText, AlertCircle, CheckCircle, FileWarning } from 'lucide-react';
import { Employee, OnCallShift, SystemUser } from '../types';
import { getEmployees, getOnCallShifts, saveOnCallShift, deleteOnCallShift } from '../services/dbService';
...
const OnCallManager: React.FC<OnCallManagerProps> = ({ currentUser }) => {
  ...
  const handleExportCSV = () => {
    ...
    const headers = [
      'Funcionário', 'Matrícula', 'Data', 'Hora Início', 'Hora Fim', 'Observação',
    ...
    link.href = url;
    link.setAttribute('download', `plantoes_smarttime_${new Date().toISOString().split('T')[0]}.csv`);
  ...
  const handleDownloadTemplate = () => {
    ...
    const exampleRow1 = ['1001', '2023-12-25', '08:00', '20:00', 'Plantão Natal'];
    const exampleRow2 = ['1002', '31/12/2023', '20:00', '08:00', 'Plantão Ano Novo'];
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onerror = () => {
        alert("Erro ao ler o arquivo. Tente novamente ou verifique se o arquivo não está corrompido.");
    };
    ...
    try {
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            if (cols.length >= 4) { 
                ...
                if (!emp) {
                    errorCount++;
                    errorsDetails.push(`Linha ${i+1}: Funcionário com matrícula '${matricula}' não encontrado.`);
                    continue;
                }
                ...
                if (/^\d{4}-\d{2}-\d{2}$/.test(dateRaw)) {
                    ...
                    finalDate = `${year}-${month}-${day}`;
                     ...
                     errorCount++;
                     errorsDetails.push(`Linha ${i+1}: Data inválida '${dateRaw}' para matrícula ${matricula}.`);
                     continue;
                }
                ...
                errorCount++;
                errorsDetails.push(`Linha ${i+1}: Formato incorreto ou colunas insuficientes.`);
            }
        }
        ...
        setImportFeedback({
            success: 0,
            errors: 1,
            details: ["Erro crítico ao processar o arquivo. Verifique o formato do CSV."]
        });
    }
  };
  ...
  return (
    <div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
          <h1 className="text-2xl font-bold text-[#1E1E1E]">Escala de Plantões</h1>
          <p className="text-slate-500">Gerencie os funcionários escalados para plantão.</p>
        ...
        <div className="flex flex-wrap gap-2 relative w-full md:w-auto">
            ...
            <button 
                ...
                title="Importar CSV: Matrícula;Data;Inicio;Fim;Obs"
            >
                <Upload size={18} />
      ...
      <div className="bg-white shadow-sm border border-slate-200 rounded-xl overflow-hidden mb-20">
        ...
        {filteredShifts.length === 0 ? (
          ...
          <div className="divide-y divide-slate-100">
            {filteredShifts.slice().sort((a,b) => b.date.localeCompare(a.date)).map(shift => {
                ...
                const auditLabel = shift.updatedBy ? 'Atualizado por' : 'Registrado por';
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md flex flex-col animate-in fade-in zoom-in duration-200">
            ...
            <div className="p-6 overflow-y-auto">
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div>
                        <label className="block text-sm font-medium text-slate-700 mb-1">Data do Plantão <span className="text-rose-500">*</span></label>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-medium text-slate-700 mb-1">Hora Início <span className="text-rose-500">*</span></label>
                            <input required type="time" className="w-full border border-slate-300 rounded-lg p-2 outline-none focus:border-[#204294] bg-white text-slate-900"
                            value={startTime} onChange={e => setStartTime(e.target.value)} />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-slate-700 mb-1">Hora Fim <span className="text-rose-500">*</span></label>
                            <input required type="time" className="w-full border border-slate-300 rounded-lg p-2 outline-none focus:border-[#204294] bg-white text-slate-900"
                    ...
                    <div>
                        ...
                        <textarea rows={3} placeholder="Ex: Sobreaviso remoto, Plantão presencial..." 
      ...
      {deleteConfirmationId && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-sm overflow-hidden p-6 text-center">
            ...
            <h3 className="text-lg font-bold text-slate-800 mb-2">Excluir Plantão?</h3>
